...
geo_eligibility: {string from `compute-fit.mjs --score`'s `geo_eligibility` — one of `india-eligible` / `global-remote` / `restricted` / `unknown`. NEVER changes `score` above — additive signal only.}
geo_evidence: {string from `compute-fit.mjs --score`'s `geo_evidence` — the specific matched location/JD text that drove the tier above, e.g. `location restricted to a non-India country/hub: "USA"`. Always present, always copy verbatim — this is what makes `geo_eligibility` inspectable instead of a black-box number.}
...
**`domain_fit_score`/`comp_effective_value_inr`/`india_hireability_confidence`/`geo_eligibility`/`geo_evidence`/`fit_rank` lookup (optional, additive-only):** before writing the Machine Summary, run `node compute-fit.mjs --score --company "{company}" --jd-text "{the JD text already read for Blocks A/B}" --location-text "{the JD's own stated location, verbatim, e.g. from the posting's location field or an explicit "Location:" line in the JD body — pass an empty string if genuinely not stated}"`, adding `--comp-amount N --comp-currency CCY` when the JD's own `advertised_comp` figure (already extracted verbatim for Block D) contains a usable number — parse it yourself into a single representative amount and currency code (e.g. the lower bound of a stated range); never invent a figure the JD doesn't state. Add `--india-entity` and/or `--mentions-eor` only when Block D's research turned up direct evidence of either. Copy the six resulting fields into the Machine Summary verbatim, including any that come back `null` — never estimate a missing one, never block the evaluation waiting on this lookup, and never let any of these fields influence `score`, `final_decision`, or any other evaluation field above.

**Geo-restriction check, do this explicitly, don't rely on the keyword heuristic alone:** the CLI lookup above is a fast regex-based approximation — it can miss subtler restriction language a full read of the JD would catch. While reading the JD for Blocks A/B, also actively look for: citizenship/work-authorization requirements ("must be a US citizen," "authorized to work in [country] without sponsorship"), a security-clearance requirement (near-guaranteed single-country restriction), or a "remote" claim that's actually scoped to one country/region once you read past the headline. If you find something the CLI's `geo_evidence` field didn't already capture, note it explicitly in Block A (Role Summary) — this is exactly the class of thing a keyword scan can miss but a careful read catches, and it matters more than almost anything else in this evaluation: the user is a remote-only, India-based senior candidate, and a role that's secretly geo-restricted is not viable regardless of how well it otherwise scores.
