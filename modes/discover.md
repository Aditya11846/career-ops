...
| `why` | ✅ | One line: why this matches the intent. This is the agent's judgment — the thing a deterministic scan can't give. **Geo-fit vocabulary (2026-08-02):** when `config/profile.yml`'s location policy is India-based/remote-only (the default — check it), and the posting's location/JD carries geo signal either way, describe it using the SAME four terms `compute-fit.mjs`'s `classifyGeoEligibility()` uses for the deterministic pipeline — `india-eligible` (India explicitly listed/EOR evidence), `global-remote` (genuinely worldwide, no country qualifier), `restricted` (scoped to one non-India country/citizenship — the common "remote" trap), `unknown` (no evidence either way). This keeps AI-search results readable in the same language as pipeline results, not a second, inconsistent vocabulary for the same underlying question. Still not a fit judgment — you're describing what the location signal says, not deciding whether to include the candidate (the generous-inclusion rule above still applies: flag `restricted`, don't discard on a shallow read). |
