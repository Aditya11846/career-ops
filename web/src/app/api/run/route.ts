...
4. **Warm-path check (do this BEFORE drafting) — the single highest-leverage step in this whole flow.** Cold outreach converts at roughly 2-8%; a genuine shared-employer/school connection converts at 40-65% (original relationship-pipeline-signals.md research). For each real person found, check their LinkedIn/bio for any employer or school that ALSO appears in Amit's own cv.md work history (Akamai, KOKO Networks, Nuance Communications, PrimaryIO, Symantec, Innominds, Azingo, Aztecsoft, Computer Associates, Synergy Infotech, Oceanus Technologies, or his university — read cv.md yourself to confirm the exact current list, don't rely on this hardcoded copy if cv.md has changed). A REAL overlap means the same employer/school genuinely appears in both — not "also worked in security" or "also an engineer," that's not an overlap, that's just the same industry.
   - If a genuine overlap exists: this is WARM, not cold. Lead the message with it explicitly (e.g. "Saw we both spent time at Symantec around 2015-2018") rather than burying it. Prefix the notes with \`WARM ({the shared employer/school}):\` before the contact-type label.
   - If no genuine overlap: proceed as cold outreach per modes/contacto.md's normal persona engine. Prefix the notes with \`COLD:\` before the contact-type label, so warm vs. cold contacts stay visually distinguishable later.
   - Never fabricate an overlap that isn't real just to make an outreach look warmer — that would be worse than honest cold outreach.
5. For each REAL person found, classify contact type and draft the message per modes/contacto.md's persona engine (≤300 chars, no corporate-speak, never share a phone number), applying the warm-path lead from step 4 when it's real.
6. BEFORE adding anyone, run \`node relationships.mjs --json\` and check if a person with the SAME name (case-insensitive) at the SAME company is already tracked (confirmed real bug 2026-07-29: re-running this without checking created a duplicate "Kent Button" entry instead of filling in the original's missing contact info). If a match exists:
...
7. NEVER send, submit, or click anything. This is research + draft-only — the user copies and sends manually themselves.

End with EXACTLY one final line: VERDICT: {number of real contacts found and added}/5 — {names + contact types + WARM/COLD, ≤20 words}
